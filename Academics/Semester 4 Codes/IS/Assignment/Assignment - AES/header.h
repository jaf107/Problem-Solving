...
void KeyExpansion(unsigned char inputKey[16], unsigned char expandedKeys[176])
{
    ...
    int rconIteration = 1;
    unsigned char temporaryCore[4];

    while(bytesGenerated < 176){

        for(int i = 0;i< 4; i++)
        {
            temporaryCore[i] = expandedKeys[i + bytesGenerated - 4];
        }

        if(bytesGenerated % 16 == 0)
        {
            KeyExpansionCore(temporaryCore,rconIteration++);
        }

        for (unsigned char a = 0; a < 4; a++)
        {
			expandedKeys[bytesGenerated] = expandedKeys[bytesGenerated - 16] ^ temporaryCore[a];
			bytesGenerated++;
		}
